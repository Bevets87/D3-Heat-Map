 ...
 // make data points rectangles and fill color will correspond to color scale
  ...
  getData(url).then(function (data) {
    if (window.innerWidth < 600) {
      drawSVG(data, {width: 350, height: 350, top: 20, bottom: 50, right: 30, left: 30})
    } else if (window.innerWidth < 1000 && window.innerWidth >= 600) {
      drawSVG(data, {width: 600, height: 400, top: 40, bottom: 80, right: 30, left: 70})
    } else {
      drawSVG(data, {width: 1000, height: 500, top: 50, bottom: 95, right: 50, left: 70})
    }

    window.addEventListener('orientationchange', function () {
      if (screen.orientation.angle === 90) {
        drawSVG(data, {width: 600, height: 400, top: 40, bottom: 80, right: 30, left: 50})
      } else {
        drawSVG(data, {width: 350, height: 350, top: 20, bottom: 50, right: 30, left: 30})
      }
    })
